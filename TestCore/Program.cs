...
namespace TestCore {
	internal class Program {
		private static void Main(string[] args) {
			...
			// testGetItem(auth);
			// testUpdateItemDelisting(auth);
			testUpdateItemQuantity(auth);
		}
		//
		...
		private static void testUpdateItemDelisting(Auth auth) {
			var result = new UpdateItemDelisting(auth, new UpdateItemListingParams {
				ItemId = 451708760
			}).Execute();
			Console.WriteLine(result);
		}
		private static void testUpdateItemListing(Auth auth) {
			var result = new UpdateItemListing(auth, new UpdateItemListingParams {
				ItemId = 451708760
			}).Execute();
			Console.WriteLine(result);
		}
